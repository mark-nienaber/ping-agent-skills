...
description: Configure the Hellō connector to add Hellō as an OpenID Connect login option in PingOne DaVinci and receive verified user claims in your flow
...
\</div>)* redirect to the user's Hellō Wallet and then returns to PingOne DaVinci with the claims you request, such as verified email, verified phone, and profile picture, so you can use them in your flow.
...
| Setting                       | Description                                                                                                                                                                    |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Redirect URI**              | The PingOne DaVinci redirect URI for this connector. This value is provided automatically as a read-only field and should be copied into your Hellō application configuration. |
...
The **Redirect to Hellō** capability redirects the user to their Hellō Wallet to authenticate, then returns to PingOne DaVinci with the requested claims.

At a high level:

1. The **Redirect to Hellō** capability sends the user to the Hellō Wallet to sign in and consent to the requested claims.

2. After the user authenticates, Hellō redirects back to the PingOne DaVinci redirect URI and the connector outputs the user claims you can evaluate in your flow.
