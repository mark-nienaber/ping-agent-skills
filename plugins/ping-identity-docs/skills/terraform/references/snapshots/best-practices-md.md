...
section_ids:
  ...
  plan-first-before-apply: plan first before apply
  use-auto-approve-with-caution: Use --auto-approve with caution
  store-state-securely: Store state securely
  dont-modify-state-directly: Don't modify state directly
  capture-provider-warnings: Ensure provider warnings are captured and reviewed
  hcl-writing-recommendations: HCL writing recommendations
  use-terraform-formatting-tools: Use Terraform formatting tools
  validate-terraform-hcl: Validate Terraform HCL before plan and apply
  using-count-and-foreach: Using count and for_each with resource iteration
  use-maps-with-static-keys: Use maps with static keys when using for_each on resources
  best-practice: Best practice
  not-best-practice: Not best practice
  write-and-publish-reusable-modules: Write and publish reusable modules
  versioning: Versioning
  use-terraform-version-control: Use Terraform version control
  use-provider-version-control: Use provider version control
  use-module-version-control: Use module version control
  ...
  variable-and-secret-management: Use of Terraform variables and secrets management
  multi-team-development: Multi-team development
  use-on-demand-development-environments: Use on-demand development environments
  cicd: Continuous Integration/Continuous Delivery (CI/CD)
  use-terraform-linting-tools: Use Terraform linting tools
  use-terraform-security-scanning-tools: Use Terraform security scanning tools
  check-lock-file-to-scm: Check the .terraform.lock.hcl file into source control
---
...
These guidelines are not intended to educate on the use of Terraform and are not a getting started guide. You can find more information about Terraform in [Hashicorp's Online Documentation](https://developer.hashicorp.com/terraform/docs). Learn how to get started with the Ping Identity Terraform providers in the [Getting Started](terraform_landing_page.html#get-started) guides.
...
Learn more about state management when using Terraform in [Hashicorp's online documentation](https://developer.hashicorp.com/terraform/language/state).
...
As a specific example, [`davinci_flow`](https://registry.terraform.io/providers/pingidentity/davinci/latest/docs/resources/flow) resources validate the [`flow_json`](https://registry.terraform.io/providers/pingidentity/davinci/latest/docs/resources/flow#flow_json) input and the specified [`connection_link`](https://registry.terraform.io/providers/pingidentity/davinci/latest/docs/resources/flow#connection_link) blocks to make sure remapped connections are valid.
...
The problem is compounded when adding and removing elements to and from the array. This scenario is an example of when to use `for_each` instead of `count`, as `for_each` will identify and store each resource with a unique key. Including guidance from the [Use maps with static keys when using `for_each` on resources](#use-maps-with-static-keys) best practice, the following HCL is the recommended way to perform the same iteration:
