...
section_ids:
  ...
  june-16: June 16
  flow-analytics-connector-enhancements: Flow Analytics connector enhancements
  june-15: June 15
  validation-messages-in-custom-html-template-nodes-now-support-localization: Validation messages in Custom HTML Template nodes now support localization
  june-12: June 12
  documentation-for-social-login-connectors: Documentation for social login connectors
  june-4: June 4
  improved-dynamic-theme-selection-in-the-form-connector: Improved dynamic theme selection in the Form connector
  june-2: June 2
...
#### DaVinci audit events in PingOne early access

New PingOne DaVinci

DaVinci administrative audit events are now available in PingOne early access. You can find them listed in **Monitoring > Audit** alongside standard PingOne events. Each event includes actor identity, client name, IP address, resource name, result status, and a DaVinci-specific event detail payload.
...
#### Improved Content Security Policy (CSP) controls
...
The PingOne AI Assistant for DaVinci can now create specific categories of flows and update the flows it created using natural language prompts. Learn more in [Using the PingOne AI Assistant for DaVinci](../flows/davinci_using_the_ai_assistant.html).

### June 16

#### Flow Analytics connector enhancements
...
* **New global variables**: Three new global variables are available for use in custom analytics events:

  * `{{global.flowName}}`: The name of the executing flow

  * `{{global.flowVersionId}}`: The version number of the executing flow

  * `{{global.flowVersionAlias}}`: The version alias of the executing flow

* **Expanded **Outcome Description/Comment** limit**: The character limit for the **Outcome Description/Comment** field has been increased from 1024 to 2500 characters.

### June 15

#### Validation messages in **Custom HTML Template** nodes now support localization

New Platform

When a user completes forms created in **Custom HTML Template** nodes, they can now receive validation rule feedback in their own language. Add language keys in the message fields in the **Form validation rules** section.
...
#### Documentation for social login connectors
...
#### Improved dynamic theme selection in the Form connector

New Connector

Previously, the **Show Form** capability allowed you to select a static theme or provide a dynamic theme ID at runtime.

A new **Use Active Theme** option applies whichever theme is currently active when the flow runs, letting you control the theming for all flows from **Branding & Themes**.

The previous **PingOne Active Theme** option is now renamed to ***\<theme name>* (Active)** and moved to a new **Static Selection** section. If the environment's active theme changes, the "(Active)" indicator will move to the correct theme in the list, but your static theme selection will not change in the node configuration.
...
The dates for the remaining CSP phases have shifted:

* Mid-June 2026: You'll be able to configure CSP overrides at the flow level if your environment uses custom domains.

* Mid-July 2026: We'll transition the baseline CSP to enforcement mode with a controlled rollout.
